...
class MainScreen:
    ...
    def clicked(self, option, row = None):
            ...
            if (option == "update" and row is not None and len(row) != 0) or option == "addnew":
    ...
    def deleteRecord(self):
        if self.row is not None and len(self.row) != 0:
            ...
            try:
                ...
                cursor = mydb.cursor()
                cursor.execute(
                "DELETE FROM accounts WHERE username = %s AND url=%s AND acc_username=%s AND acc_password=%s AND site=%s",
                (self.login_username, u, usr, p, s)
                )
                mydb.commit()
                self.displayData()
            except Error as err:
